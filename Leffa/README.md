...
具体地，对于生成式UNet $ε_{θ_{gen}} (·)$ 的第 $l$ 层注意力层，使用 $F^{l}_{gen}$ 作为查询 $Q$，$F^{l}_{ref}$ 作为键 $K$ 来计算注意力图（attention map） $A^l$ ，公式表示的点积注意力如下:
